...
```
java -ea -Djava.library.path=/opt/cloudhsm/lib/ -jar target/assembly/keystore.jar --user <CU User> --password <CU Password> --partition <partition> 
```

## View keys from CloudHSM

Before running the command, make sure to export the credentials as environment variables:
```
export HSM_PASSWORD=<CU Password>
export HSM_USER=<CU User>
export HSM_PARTITION=<Partition>
```

Then run the command below:

```
java -ea -Djava.library.path=/opt/cloudhsm/lib/ -jar target/assembly/key-utility-runner.jar --get-all-keys
```
